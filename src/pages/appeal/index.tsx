...
import { explain, getOrderDetailsById, handleBuy } from '@/apis'
...
const Home = () => {
  ...
  const handlePush = () => {
    explain({
      explainReason: appealText,
      tid: orderData.id,
      urls: [imageUrl],
    })
      .then((res: any) => {
        if (res.code === 200) {
          message.success('申訴提交成功')
          setTimeout(() => {
            navigate(-1)
          }, 3000)
        } else {
  ...
  const handleChange: UploadProps['onChange'] = (info: UploadChangeParam<UploadFile>) => {
    ...
    if (info.file.status === 'done') {
      localStorage.setItem('appealImg', info.file.response.data)
  ...
  const handleChangeBotValue = (val: string) => {
    setAppealText(val)
  }
    ...
    <LayoutElement>
      <div className="uploadWarp">
        <div className="uploadWarpTop">
          ...
          />
          <p className="uploadWarpTopTitle">申訴</p>
          <div></div>
        </div>
        <input
          type="text"
          className="appealIpt"
          onChange={(e) => {
            handleChangeBotValue(e.target.value)
          }}
          value={appealText}
        />
