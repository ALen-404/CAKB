...
import { cancelBuy, cancelSale, discharg, getOrderDetailsById, handleBuy, handleUploadPaymentVoucher } from '@/apis'
...
const Home = () => {
  ...
  const handleCancelOrder = () => {
    if (address === orderData.fromAddress) {
      cancelSale({
        id: orderData.id,
      })
        .then((res: any) => {
            ...
            {orderData.status !== 5 && (
              <div className="orderDetailsWarpConFooter">
                ...
                {orderData.fromAddress === address ? (
                  ...
                  <>
                    {(orderData?.status === 3 || orderData?.status === 4) && (
                      <Link to={'/uploaded'}>
                        <div className="ConFooterRight">
                          {orderData.paymentUrls ? <p className="pushed">已上傳</p> : <p>未上傳</p>}
          ...
          <>
            {orderData?.status === 3 && (
              <div className="confirmBuy">
                ...
                <Button onClick={releasePayment} className={'sureBtn'}>
                  放行
                </Button>
              </div>
            )}
