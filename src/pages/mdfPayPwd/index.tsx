...
import { getSellTradingByUser, getTopUpInfo, getTradingByUser, getUserInfo, setPassword, updatePassword } from '@/apis'
...
const Home = () => {
  ...
  const [oldPayPwdAddress, setoldPayPwdAddress] = useState('')
  ...
  const setPwdHandle = () => {
    ...
    updatePassword({
      newPassword: oldPayPwdAddress,
      oldPassword: payPwdAddress,
    })
      .then((res: any) => {
        if (res.code === 200) {
        ...
        <div className="payPwdContent">
          <div className="payPwdContentItem">
            ...
            <input
              type="password"
              onChange={(e: any) => {
                setoldPayPwdAddress(e.target.value)
              }}
              value={oldPayPwdAddress}
            />
          </div>
          <div className="payPwdContentItem">
            <p>新交易密碼</p>
