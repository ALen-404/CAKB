...
const Home = () => {
        ...
        .then((res: any) => {
          if (res.code === 200) {
            const initData = res.data.slice(0, 5).map((item: any) => {
              ...
              const date = new Date(parsedItem.time)
              const formattedDate = `${date.getFullYear()} ${String(date.getMonth() + 1).padStart(2, '0')}-${String(
                date.getDate()
              ).padStart(2, '0')} `

              return { time: date, type: String(parsedItem.type) }
            })
  ...
  const config = {
    ...
    axis: {
      ...
      y: {
        labelFormatter: (text: any) => (Number(text) === 1 ? '漲' : '跌'),
      },
    },
    tooltip: {
      title: '漲跌',
      items: [{ channel: 'y', valueFormatter: (d: any) => (Number(d) === 1 ? '漲' : '跌') }],
    },
    smooth: true,
  }
    ...
    <LayoutElement>
      <div className="gussItemBox">
        <div className="gussItemTop">
          ...
          <div className="balance">{Number(userInfo?.usable || '0').toFixed(2)}</div>
